// ============================================================
// NODE CODE ULTRA ROBUSTE — Programme contrôle comptes
// Gère TOUS les formats possibles de sortie LLM
// ============================================================

console.log('🚀 [NODE CODE] Démarrage du traitement...');

// 1. Récupérer TOUTES les données disponibles
const items = $input.all();
console.log(`📦 [NODE CODE] Nombre d'items reçus: ${items.length}`);
...
for (let i = 0; i < items.length; i++) {
  const item = items[i];
  console.log(`🔍 [NODE CODE] Analyse item ${i}:`, Object.keys(item.json || {}));
  ...
  if (item.json) {
    ...
    if (item.json.data) {
      console.log('✅ [NODE CODE] Données déjà dans format data');
      return [{
        json: {
          data: item.json.data
        }
...
console.log(`📍 [NODE CODE] Source trouvée: ${sourceType}`);
console.log(`📏 [NODE CODE] Longueur: ${rawOutput.length} caractères`);
console.log(`👀 [NODE CODE] Aperçu (200 chars):`, rawOutput.substring(0, 200));
...
if (jsonFenceMatch) {
  ...
  console.log('🧹 [NODE CODE] Bloc markdown retiré');
}
...
if (startPos > 0 || endPos < jsonStr.length) {
  if (startPos !== -1 && endPos !== -1) {
    ...
    console.log('🧹 [NODE CODE] Texte parasite retiré');
  }
}
...
console.log('✨ [NODE CODE] JSON nettoyé');
...
try {
  ...
  console.log('✅ [NODE CODE] Parse JSON réussi');
} catch (err1) {
  console.warn('⚠️ [NODE CODE] Parse échoué (tentative 1):', err1.message);
  
  try {
    const cleanStr = jsonStr.replace(/[\u0000-\u001F\u007F-\u009F]/g, ' ');
    parsedData = JSON.parse(cleanStr);
    console.log('✅ [NODE CODE] Parse JSON réussi (tentative 2)');
  } catch (err2) {
    console.error('❌ [NODE CODE] Parse échoué (tentative 2):', err2.message);
    
    // Retourner une structure d'erreur valide
    return [{
      json: {
        data: {
          "Tableau entete": {
...
console.log('📊 [NODE CODE] Type parsé:', Array.isArray(parsedData) ? 'Array' : 'Object');
...
if (Array.isArray(parsedData)) {
  console.log(`📋 [NODE CODE] Array de ${parsedData.length} élément(s)`);
  ...
  console.log('🔀 [NODE CODE] Objets fusionnés');
}
// CAS B: Objet direct avec les 2 clés
// {"Tableau entete": {...}, "Tableau processus": [...]}
else if (parsedData && typeof parsedData === 'object') {
  console.log('📦 [NODE CODE] Objet direct');
  finalData = parsedData;
}

// 5. Vérifier et normaliser les noms de clés
const keys = Object.keys(finalData);
console.log('🔑 [NODE CODE] Clés trouvées:', keys);
...
if (!hasTableauEntete) {
  ...
  if (enteteKey) {
    ...
    console.log(`🔄 [NODE CODE] Clé renommée: "${enteteKey}" → "Tableau entete"`);
  }
}
...
if (!hasTableauProcessus) {
  ...
  if (processusKey) {
    ...
    console.log(`🔄 [NODE CODE] Clé renommée: "${processusKey}" → "Tableau processus"`);
  }
}

// 6. Validation finale
console.log('🎯 [NODE CODE] Validation:', {
  hasTableauEntete,
  hasTableauProcessus,
...
if (!hasTableauEntete || !hasTableauProcessus) {
  console.warn('⚠️ [NODE CODE] Structure incomplète !');
  console.warn('Clés disponibles:', Object.keys(finalData));
}

// 7. Retourner au FORMAT 4 : [{ data: {...} }]
const output = [{
  json: {
    data: finalData
  }
...
console.log('✅ [NODE CODE] Format final généré');
console.log('📤 [NODE CODE] Structure retournée:', {
  isArray: true,
